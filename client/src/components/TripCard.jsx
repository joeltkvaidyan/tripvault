...
// readOnly=true hides the Edit/Delete actions — used on public profile pages
function TripCard({ trip, onEdit, onDelete, readOnly = false }) {
  ...
  return (
    <div style={styles.card}>
      <Link to={`/trips/${trip._id}`} style={styles.linkArea}>
        {trip.coverImage && <img src={trip.coverImage} alt={trip.title} style={styles.coverImage} />}
...
const styles = {
  ...
  linkArea: { color: 'inherit', textDecoration: 'none', display: 'block' },
  coverImage: {
    width: '100%',
    height: '160px',
    objectFit: 'cover',
  ...
  actions: { display: 'flex', gap: '0.5rem', padding: '0 1.25rem 1.25rem' },
  editButton: {
    flex: 1,
    padding: '0.5rem',
    background: '#2563eb',
